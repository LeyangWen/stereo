...
def stereo_calibrate(left_file, right_file, left_dir, left_prefix, right_dir, right_prefix, image_format, save_file, square_size, width=9, height=6):
    ...
    global image_size
    if False:
        # to make it faster to debug, we can store the intermediate result
        objp, leftp, rightp = load_image_points(left_dir, left_prefix, right_dir, right_prefix, image_format, square_size, width, height)
        pickle.dump([objp, leftp, rightp],open(save_file+'checker.p','wb'))
    else:
        objp1, leftp1, rightp1 = pickle.load(open(save_file+'checker.p','rb'))
    ...
    total_pair = len(objp1)
    objp = []
    leftp =[]
    rightp = []
    if False:
        # use this to findout bad pairs and get rid of them
        for n in range(total_pair-1,total_pair):
            objp = objp1[:n]
            leftp = leftp1[:n]
            rightp = rightp1[:n]
            ret, K1, D1, K2, D2, R, T, E, F = cv2.stereoCalibrate(objp, leftp, rightp, K1, D1, K2, D2, image_size, flags=flag)
            print(n,ret)
        set = (29,30,39,40)
        objp = []
        leftp =[]
        rightp = []
        for n in range(total_pair):
            if n in set:
                print(n)
                continue
            objp.append(objp1[n])
            leftp.append(leftp1[n])
            rightp.append(rightp1[n])
    else:
        objp = objp1
        leftp = leftp1
        rightp = rightp1
    ...
    R1, R2, P1, P2, Q, roi_left, roi_right = cv2.stereoRectify(K1, D1, K2, D2, image_size, R, T, alpha=0.84, flags=cv2.CALIB_ZERO_DISPARITY)
    # wen: last time this alpha affected the scale and cost me 5 hours
    # -1 or [0,1]
...
def load_image_points(left_dir, left_prefix, right_dir, right_prefix, image_format, square_size, width=9, height=6):
    ...
    for left_im, right_im in pair_images:
        ...
        if ret_left and ret_right:  # If both image is okay. Otherwise we explain which pair has a problem and continue
            ...
            print("Chessboard couldn't detected. Image: ", right_im)
        elif ret_right:
            print("Chessboard couldn't detected. Image: ", left_im)
        else:
